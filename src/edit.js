        ...
        const handleMouseMove = (e) => {
            ...
            if (Math.abs(dx) > 3 || Math.abs(dy) > 3) {
                ...
                if (!isDragging.current) {
                    ...
                    setTimeout(() => {
                        if (dragStart.current) { // Still dragging
...
function DraggableMarker({ position, onDragEnd, label }) {
    ...
    return (
        ...
        >
            <Popup>
                {label || `Lat: ${position.lat.toFixed(5)}, Lon: ${position.lng.toFixed(5)}`}
            </Popup>
        </Marker>
    );
}
...
export default function Edit({ attributes, setAttributes }) {
    ...
    const saveAsDefault = async () => {
        try {
            console.log('Saving defaults:', { sizePreset, height, width, zoom, latitude, longitude, markerLat, markerLon, markerLabel });
        ...
        <>
            <InspectorControls>
                <PanelBody title="Map Settings" initialOpen={true}>
                    ...
                    {markerPosition && (
                        <div style={{ marginTop: '12px', padding: '12px', background: '#f0f0f0', borderRadius: '4px' }}>
                            <p style={{ margin: '0 0 8px 0', fontWeight: 'bold' }}>
                                Marker Position {isLoadingAddress && <span style={{ fontSize: '11px', fontWeight: 'normal' }}>(loading address...)</span>}
                            </p>

                            {markerAddress && (
                                <div style={{ marginBottom: '12px', padding: '8px', background: '#e8f4f8', borderRadius: '4px', border: '1px solid #0073aa' }}>
                    ...
                    <div style={{ marginTop: '16px', padding: '10px', background: '#fff3cd', borderRadius: '4px', border: '1px solid #ffc107' }}>
